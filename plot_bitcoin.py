...
plt.yticks(range(0, ymax + 1, step), fontsize=13)
...
plt.title(f"Bitcoin market price in the period from {first_month} and {last_month}", fontsize=15)
...
for i in range(1, len(prices) - 1):
    if prices[i] > (prices[i-1] + epsilon) and prices[i] > (prices[i+1] + epsilon):
        plt.text(months[i], prices[i], prices[i], fontsize=13)
plt.text(months[0], prices[0], prices[0], fontsize=13)
plt.text(months[-1], prices[-1], prices[-1], fontsize=13)
